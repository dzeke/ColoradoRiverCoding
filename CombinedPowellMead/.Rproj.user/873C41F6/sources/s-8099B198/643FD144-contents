...
dfKeyTraceLabels$LabelFlex[2] <- "Private Accounts"
...
ggplot() +
  ...
  #Labels for the areas
  geom_text(data=dfKeyTraceLabels %>% filter(LabelFlex != dfKeyTraceLabels$LabelFlex[3]), aes(x=as.Date(xPositionFlex), y=VolumeFlex, label=as.character(LabelFlex)), size = 6, fontface="bold") +
  #geom_text(data=dfKeyTraceLabels %>% filter(Label == dfKeyTraceLabels$Label[3]), aes(x=as.Date(sprintf("%.0f-01-01",xPosition)), y=Volume, label=as.character(Label)), size = 4.5, fontface="bold", color = pBlues[5]) +
