---
title: "Adapt Lake Mead releases to inflow to give managers more flexibility to slow reservoir draw down"
...
This is an R Markdown document. This work supports the piece "[Adapt Lake Mead releases to inflow to give managers more flexibility to slow reservoir draw down](https://raw.githubusercontent.com/dzeke/ColoradoRiverCoding/main/MeadInflowSimulations/MeadSteadyInflowSimulation.pdf)."  There are simulations of reservoir draw down for inflow scenarios from 7 to 14 maf per year every year. These simulations assume states and contractors do not withdraw from their conservation accounts nor convert and conservation account balances to meet mandatory conservation targets. Another plot shows the reservoir release needed to stablize reservoir level for different inflow scenarios. And a final plot shows reservoir recovery from a Lake Mead elevation of 1,050 feet. Key plots include:
...
2. A line plot shows evolution of Lake Mead Active storage (y-axis) over time (x-axis) for mandatory conservation with different steady inflow scenarios. Pink area denotes the storage volumes/elevations of mandatory conservation target. Red area denotes lower storage. Simulations assume max DCP cutback in red area below 1,025 feet. We hope that if reservoir level gets that low the Lower Basin States and Mexico will cut back release further (depends on inflow) to stabilize reservoir level.

3. The Lake Mead release needed to stabilize reservoir storage at different reservoir elvations and inflows. This release is calculated by resolving the reservoir mass balance equation for release = inflow - evaporation.

4. Lake Mead recovery simulations from two elevation 1,050 feet for different reservoir inflows and additional conservation efforts beyond mandatory targets.

Data from 2019 Lower Basin Drought Contingency Plan (DCP), U.S.-Mexico Minute 323, and CRSS.  


## Requested Citation
David E. Rosenberg (2021), "Add reservoir inflow as new criteria to give Lake Mead managers more flexibility and independence to conserve water." Utah State University. Logan, Utah. https://github.com/dzeke/ColoradoRiverFutures/tree/master/MeadInflowSimulation.
...
#Package to allow knitting to a pdf
if (!require(tinytex)) { 
  install.packages("tinytex", repo="http://cran.r-project.org")
  library(tinytex) 
}

#Use to debug Tinytex
...
sMeadAug2021 <- 9.0*1e6 # Aug 30, 2021. 1,068 feet. http://mead.uslakes.info/level.asp
sMeadStartStorage <- sMeadAug2021
...
for (tInflow in c(7, 8, 8.4, 8.6, 9, 10, 11, 12,14)*1e6){
...
dfTimeResultsEven <- dfTimeResults[seq(4,nrow(dfTimeResults),by=8),]
...
ggplot() +
  ...
  #Inflow traces
  geom_line(data=dfTimeResults,aes(x=Year,y=Storage/1e6, group = Inflow/1e6, color = (Inflow/1e6)), size=1.1) +
  ...
  #scale_y_continuous(breaks = seq(0,tMaxVol,by=5), labels = seq(0,tMaxVol,by=5), limits = c(0, tMaxVol), sec.axis = sec_axis(~. +0, name = "Mead Level (feet)", breaks = dfMeadPoolsPlot$stor_maf, labels = dfMeadPoolsPlot$labelSecY)) +
  ...
  scale_color_continuous(low = pBlues[6], high=pBlues[9]) +
...
## Figure 4. Lake Mead recovery from elevation 1,050 feet. Numeric line labels (maf per year) indicate the sum of reservoir inflow and additional conservation beyond mandatory targets.
...
ggplot() +
  ...
  #geom_line(data=dfTimeResultsInteger,aes(x=Year,y=Storage/1e6), color = cRecoveryColors[3], size=2) +
    ...
    #Recovery case 1 (Elevation 1,025 feet)
  ...
  #geom_label(data=dfTimeResultsInterval %>% filter(Inflow == 9*1e6, Year < 2030), aes( x = Year, y = Storage/1e6, label = Inflow/1e6, fontface="bold"),  color = cRecoveryColors[2], size=5, angle = 0) + 
   ...
   #Label the recovery case 1 traces (Elevation 1,025 feet)
